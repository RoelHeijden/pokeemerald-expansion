...
void ItemUseInBattle_BagMenu(u8 taskId)
{
    ...
    else
    {
        ...
        if (!(B_TRY_CATCH_TRAINER_BALL >= GEN_4 && (ItemId_GetBattleUsage(gSpecialVar_ItemId) == EFFECT_ITEM_THROW_BALL) && (gBattleTypeFlags & BATTLE_TYPE_TRAINER))){
            if(gSpecialVar_ItemId != ITEM_FLUFFY_TAIL){
                RemoveUsedItem();
            }
        }
...
// ADDED
void ItemUseOutOfBattle_CannotUse2(u8 taskId)
{
    DisplayCannotUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem, gText_cannotLearnHM);
}
