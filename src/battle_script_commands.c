...
static const u16 sNaturePowerMoves[BATTLE_TERRAIN_COUNT] =
{
    ...
    [BATTLE_TERRAIN_CAVE]       = MOVE_POWER_GEM, // CHANGED
...
static const u8 sTerrainToType[BATTLE_TERRAIN_COUNT] =
{
    ...
    [BATTLE_TERRAIN_PLAIN]            = TYPE_NORMAL,  // CHANGED
};
...
static void Cmd_ppreduce(void)
{
    ...
    if (gCurrentMove == MOVE_ASSIST || gCurrentMove == MOVE_COPYCAT){
    ...
    do
    {
        ...
        case MOVEEND_UPDATE_LAST_MOVES:
            ...
            if (!gSpecialStatuses[gBattlerAttacker].dancerUsedMove)
            {
                ...
                {
                    // remove
                    // DebugPrintf("check - mon: %d, move: %d", gBattlerAttacker, gChosenMove);
...
static u32 GetTrainerMoneyToGive(u16 trainerId)
{
    ...
    if(trainerId == TRAINER_CUSTOM_TRAINER5){
        moneyReward = 500;  // RICH BOY - money battle
    }
...
{
    // CHANGED: made an exception for Snowball
    if ((!gBattleStruct->itemLost[B_SIDE_PLAYER][gBattlerPartyIndexes[battler]].stolen || itemId == ITEM_SNOWBALL)
    ...
    else
    {
        ...
        // ADDED THIS
         ...
         {   
            ...
            if (CriticalCapture(odds))  // useless code. Otherwise I get warnings that the method isn't used lol.
                shakes = 0;
