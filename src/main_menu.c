...
static void Task_MainMenuCheckSaveFile(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        tItemCount = tMenuType + 1; // CHANGED
    }
}
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (gTasks[taskId].tMenuType)
        {
            ...
            case HAS_SAVED_GAME:
                ...
                // FillWindowPixelBuffer(4, PIXEL_FILL(0xA));  // all changed: remove OPTIONS from start menu
    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (action)
        {
            ...
            default:
                ...
                // gTasks[taskId].func = Task_NoIntro_Init;  BirchIntro disabled
                break;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tLotadSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            NewGameBirchSpeech_StartFadeOutTarget1InTarget2(taskId, 0); // CHANGED to 0 from 2
            // NewGameBirchSpeech_StartFadePlatformIn(taskId, 1);
            gTasks[taskId].tTimer = 8; // CHANGED from 64 to 8
